...
if ( ! function_exists( 'ann_storage_get' ) ) {
	/**
	 ...
	 */
	function ann_storage_get( $var_name, $default = '' ) {
		global $ANN_STORAGE;
		return isset( $ANN_STORAGE[ $var_name ] ) ? $ANN_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'ann_storage_set' ) ) {
	/**
	 ...
	 */
	function ann_storage_set( $var_name, $value ) {
		global $ANN_STORAGE;
		$ANN_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'ann_storage_empty' ) ) {
	/**
	 ...
	 */
	function ann_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $ANN_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $ANN_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $ANN_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $ANN_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'ann_storage_isset' ) ) {
	/**
	 ...
	 */
	function ann_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $ANN_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $ANN_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $ANN_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $ANN_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'ann_storage_unset' ) ) {
	/**
	 ...
	 */
	function ann_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $ANN_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $ANN_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $ANN_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $ANN_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'ann_storage_inc' ) ) {
	/**
	 ...
	 */
	function ann_storage_inc( $var_name, $value = 1 ) {
		global $ANN_STORAGE;
		if ( empty( $ANN_STORAGE[ $var_name ] ) ) {
			$ANN_STORAGE[ $var_name ] = 0;
		}
		$ANN_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'ann_storage_concat' ) ) {
	/**
	 ...
	 */
	function ann_storage_concat( $var_name, $value ) {
		global $ANN_STORAGE;
		if ( empty( $ANN_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'ann_storage_get_array' ) ) {
	/**
	 ...
	 */
	function ann_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $ANN_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $ANN_STORAGE[ $var_name ][ $key ] ) ? $ANN_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $ANN_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $ANN_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'ann_storage_set_array' ) ) {
	/**
	 ...
	 */
	function ann_storage_set_array( $var_name, $key, $value ) {
		global $ANN_STORAGE;
		if ( ! isset( $ANN_STORAGE[ $var_name ] ) ) {
			$ANN_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$ANN_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'ann_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function ann_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $ANN_STORAGE;
		if ( ! isset( $ANN_STORAGE[ $var_name ] ) ) {
			$ANN_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $ANN_STORAGE[ $var_name ][ $key ] ) ) {
			$ANN_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$ANN_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$ANN_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'ann_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function ann_storage_merge_array( $var_name, $key, $value ) {
		global $ANN_STORAGE;
		if ( ! isset( $ANN_STORAGE[ $var_name ] ) ) {
			$ANN_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$ANN_STORAGE[ $var_name ] = array_merge( $ANN_STORAGE[ $var_name ], $value );
		} else {
			$ANN_STORAGE[ $var_name ][ $key ] = array_merge( $ANN_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'ann_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function ann_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $ANN_STORAGE;
		if ( ! isset( $ANN_STORAGE[ $var_name ] ) ) {
			$ANN_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			ann_array_insert_after( $ANN_STORAGE[ $var_name ], $after, $key );
		} else {
			ann_array_insert_after( $ANN_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'ann_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function ann_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $ANN_STORAGE;
		if ( ! isset( $ANN_STORAGE[ $var_name ] ) ) {
			$ANN_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			ann_array_insert_before( $ANN_STORAGE[ $var_name ], $before, $key );
		} else {
			ann_array_insert_before( $ANN_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'ann_storage_push_array' ) ) {
	/**
	 ...
	 */
	function ann_storage_push_array( $var_name, $key, $value ) {
		global $ANN_STORAGE;
		if ( ! isset( $ANN_STORAGE[ $var_name ] ) ) {
			$ANN_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $ANN_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $ANN_STORAGE[ $var_name ][ $key ] ) ) {
				$ANN_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $ANN_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'ann_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function ann_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $ANN_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $ANN_STORAGE[ $var_name ] ) && is_array( $ANN_STORAGE[ $var_name ] ) && count( $ANN_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $ANN_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $ANN_STORAGE[ $var_name ][ $key ] ) && is_array( $ANN_STORAGE[ $var_name ][ $key ] ) && count( $ANN_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $ANN_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'ann_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function ann_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $ANN_STORAGE;
		if ( ! isset( $ANN_STORAGE[ $var_name ] ) ) {
			$ANN_STORAGE[ $var_name ] = array();
		}
		if ( empty( $ANN_STORAGE[ $var_name ][ $key ] ) ) {
			$ANN_STORAGE[ $var_name ][ $key ] = 0;
		}
		$ANN_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'ann_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function ann_storage_concat_array( $var_name, $key, $value ) {
		global $ANN_STORAGE;
		if ( ! isset( $ANN_STORAGE[ $var_name ] ) ) {
			$ANN_STORAGE[ $var_name ] = array();
		}
...
if ( ! function_exists( 'ann_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function ann_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $ANN_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ANN_STORAGE[ $var_name ] ) ? $ANN_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ANN_STORAGE[ $var_name ] ) ? $ANN_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'ann_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function ann_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $ANN_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $ANN_STORAGE[ $var_name ]->$prop ) ? $ANN_STORAGE[ $var_name ]->$prop : $default;
	}
}
