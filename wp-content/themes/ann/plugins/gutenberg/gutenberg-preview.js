...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			ann_create_observer( 'ann-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				ann_gutenberg_editor_init();
			} );
		} else {
		 	ann_gutenberg_editor_init();
		}
	...
	function ann_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'ann_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'ann_inited' ) ) {
	...
	// Init on page load
	function ann_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = ann_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + ANN_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + ANN_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in ANN_STORAGE['override_classes'] ) {
				$obj.addClass( ANN_STORAGE['override_classes'][i].replace( '%s', ANN_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( ann_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && ANN_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'ann_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			ann_remove_observer( 'ann-check-editor-styles-wrapper' );
			ann_create_observer( 'ann-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'ann_inited' );
	}
} );
