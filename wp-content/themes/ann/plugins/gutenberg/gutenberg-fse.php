...
if ( ! defined( 'ANN_FSE_TEMPLATE_PART_PT' ) ) define( 'ANN_FSE_TEMPLATE_PART_PT', 'wp_template_part' );

if ( ! defined( 'ANN_FSE_TEMPLATE_PART_AREA_HEADER' ) )  define( 'ANN_FSE_TEMPLATE_PART_AREA_HEADER',  defined( 'WP_TEMPLATE_PART_AREA_HEADER' )  ? WP_TEMPLATE_PART_AREA_HEADER  : 'header' );
if ( ! defined( 'ANN_FSE_TEMPLATE_PART_AREA_FOOTER' ) )  define( 'ANN_FSE_TEMPLATE_PART_AREA_FOOTER',  defined( 'WP_TEMPLATE_PART_AREA_FOOTER' )  ? WP_TEMPLATE_PART_AREA_FOOTER  : 'footer' );
if ( ! defined( 'ANN_FSE_TEMPLATE_PART_AREA_SIDEBAR' ) ) define( 'ANN_FSE_TEMPLATE_PART_AREA_SIDEBAR', defined( 'WP_TEMPLATE_PART_AREA_SIDEBAR' ) ? WP_TEMPLATE_PART_AREA_SIDEBAR : 'sidebar' );
if ( ! defined( 'ANN_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) define( 'ANN_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED', defined( 'WP_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ? WP_TEMPLATE_PART_AREA_UNCATEGORIZED : 'uncategorized' );

// Load additional files with FSE support
require_once ann_get_file_dir( 'plugins/gutenberg/gutenberg-fse-lists.php' );
require_once ann_get_file_dir( 'plugins/gutenberg/gutenberg-fse-templates.php' );


if ( ! function_exists( 'ann_gutenberg_fse_loader' ) ) {
	add_action( 'wp_loaded', 'ann_gutenberg_fse_loader', 1 );
	/**
	 * Turn off a Gutenberg (plugin) templates on frontend
	 * if a theme-specific frontend editor is active.
	 */
	function ann_gutenberg_fse_loader() {
		// 
		if ( ann_exists_gutenberg_fse()
			&& get_option( 'show_on_front' ) == 'page'
			&& (int) get_option('page_on_front') > 0
			&& ann_is_on( ann_get_theme_option( 'front_page_enabled', false ) )
			&& ann_get_current_url() == '/'
		) {
...
if ( ! function_exists( 'ann_gutenberg_is_fse_wp_support' ) ) {
	/**
	 * Check if WordPress with FSE support installed
	 * (WordPress 5.9+ Site Editor is present).
	 */
	function ann_gutenberg_is_fse_wp_support() {
		return function_exists( 'wp_is_block_theme' );
	}
}

if ( ! function_exists( 'ann_gutenberg_is_fse_gb_support' ) ) {
	/**
	 * Check if a plugin "Gutenberg" with FSE support is installed.
	 */
	function ann_gutenberg_is_fse_gb_support() {
		return function_exists( 'gutenberg_is_fse_theme' );
	}
}

if ( ! function_exists( 'ann_exists_gutenberg_fse' ) ) {
	/**
	 * Check if FSE is available
	 * (WordPress 5.9+ or the plugin "Gutenberg" are installed).
	 */
	function ann_exists_gutenberg_fse() {
		return ann_exists_gutenberg() && ( ann_gutenberg_is_fse_wp_support() || ann_gutenberg_is_fse_gb_support() );
	}
}

if ( ! function_exists( 'ann_gutenberg_is_fse_theme' ) ) {
	/**
	 * Check if the current theme supports a FSE behaviour.
	 */
	function ann_gutenberg_is_fse_theme() {
		static $fse = -1;
		if ( $fse == -1 ) {
			if ( ann_gutenberg_is_fse_wp_support() ) {                                    // WordPress 5.9+ FSE
				$fse = (bool) wp_is_block_theme();
			} else if ( ann_gutenberg_is_fse_gb_support() ) {                             // Plugin Gutenberg FSE
				$fse = (bool) gutenberg_is_fse_theme();
			} else {
				$fse = is_readable( ANN_THEME_DIR . 'templates/index.html' )              // WordPress 5.9+ FSE
						|| ( ANN_THEME_DIR != ANN_CHILD_DIR && is_readable( ANN_CHILD_DIR . 'templates/index.html' ) )
						|| is_readable( ANN_THEME_DIR . 'block-templates/index.html' )    // Plugin Gutenberg FSE
						|| ( ANN_THEME_DIR != ANN_CHILD_DIR && is_readable( ANN_CHILD_DIR . 'block-templates/index.html' ) );
			}
		}
		return $fse;
	}
...
if ( ! function_exists( 'ann_gutenberg_is_fse_enabled' ) ) {
	/**
	 * Check if the current theme supports FSE behaviour and FSE is installed and activated
	 */
	function ann_gutenberg_is_fse_enabled() {
		return ann_exists_gutenberg_fse() && ann_gutenberg_is_fse_theme();
	}
}

if ( ! function_exists( 'ann_gutenberg_fse_skin_updated' ) ) {
	add_action( 'ann_action_theme_updated', 'ann_gutenberg_fse_skin_updated' );
	add_action( 'ann_action_skin_updated', 'ann_gutenberg_fse_skin_updated', 10, 1 );
	/**
	 * Copy FSE-required folders from an updated skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'ann_action_skin_updated', 'ann_gutenberg_fse_skin_updated', 10, 1 );
	 */
	function ann_gutenberg_fse_skin_updated( $skin = '' ) {
		// Get an active skin name if empty
		if ( current_action() == 'ann_action_theme_updated' && empty( $skin ) ) {
			$skin = ann_skins_get_active_skin_name();
		}
		if ( ann_skins_get_current_skin_name() == $skin ) {
			ann_gutenberg_fse_copy_from_skin( $skin );
			ann_gutenberg_fse_update_theme_json( $skin );
		}
	}
}

if ( ! function_exists( 'ann_gutenberg_fse_copy_from_skin' ) ) {
	add_action( 'ann_action_skin_switched', 'ann_gutenberg_fse_copy_from_skin', 10, 2 );
	/**
	 * Copy FSE-required folders from a new skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'ann_action_skin_switched', 'ann_gutenberg_fse_copy_from_skin', 10, 2 );
	 */
	function ann_gutenberg_fse_copy_from_skin( $new_skin, $old_skin = '' ) {
		$theme_templates_dir = ann_prepare_path( ANN_THEME_DIR . ( ann_gutenberg_is_fse_wp_support() ? 'templates/' : 'block-templates/' ) );
		$theme_template_parts_dir = ann_prepare_path( ANN_THEME_DIR . ( ann_gutenberg_is_fse_wp_support() ? 'parts/' : 'block-template-parts/' ) );
		$theme_json = ann_prepare_path( ANN_THEME_DIR . 'theme.json' );
		$skin_templates_dir = ann_prepare_path( ANN_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( ann_gutenberg_is_fse_wp_support() ? '/templates/' : '/block-templates/' ) );
		$skin_template_parts_dir = ann_prepare_path( ANN_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( ann_gutenberg_is_fse_wp_support() ? '/parts/' : '/block-template-parts/' ) );
		// Remove old templates from the stylesheet dir (if exists)
		if ( is_dir( $theme_templates_dir ) ) {
			ann_unlink( $theme_templates_dir );
		}
		if ( is_dir( $theme_template_parts_dir ) ) {
			ann_unlink( $theme_template_parts_dir );
		}
		if ( file_exists( $theme_json ) ) {
			ann_unlink( $theme_json );
		}
		...
		if ( is_dir( $skin_templates_dir ) ) {
			ann_copy( $skin_templates_dir, $theme_templates_dir );
		}
		if ( is_dir( $skin_template_parts_dir ) ) {
			ann_copy( $skin_template_parts_dir, $theme_template_parts_dir );
		}
	}
}

if ( ! function_exists( 'ann_gutenberg_fse_update_theme_json' ) ) {
	add_action( 'ann_action_skin_switched', 'ann_gutenberg_fse_update_theme_json', 30, 2 );
	add_action( 'ann_action_save_options', 'ann_gutenberg_fse_update_theme_json', 30 );
	add_action( 'trx_addons_action_save_options', 'ann_gutenberg_fse_update_theme_json', 30 );
	/**
	 * Update the file 'theme.json' after the current skin is switched and/or theme options are saved.
	 * 
	 * Hooks: add_action( 'ann_action_skin_switched', 'ann_gutenberg_fse_update_theme_json', 30, 2 );
	 * 
	 *        add_action( 'ann_action_save_options', 'ann_gutenberg_fse_update_theme_json', 30 );
	 * 
	 *        add_action( 'trx_addons_action_save_options', 'ann_gutenberg_fse_update_theme_json', 30 );
	 * 
	 * Trigger filter 'ann_filter_gutenberg_fse_theme_json_data' to allow other modules modify this data before saving.
	 */
	function ann_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// Get an active skin name if empty
		if ( in_array( current_action(), array( 'ann_action_save_options', 'trx_addons_action_save_options' ) ) && empty( $new_skin ) ) {
			$new_skin = ann_skins_get_active_skin_name();
		}
		// If the current skin supports FSE
		if ( ! empty( $new_skin ) && is_dir( ANN_THEME_DIR . ann_skins_get_current_skin_dir( $new_skin ) . '_fse' ) ) {
			$theme_json_data = ann_gutenberg_fse_theme_json_data();
			if ( ! empty( $theme_json_data ) && is_array( $theme_json_data ) ) {
				ann_fpc( ann_prepare_path( ANN_THEME_DIR . 'theme.json' ), json_encode( $theme_json_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) );
			}
		}
	}
}

if ( ! function_exists( 'ann_gutenberg_fse_theme_json_data' ) ) {
	/**
	 * Return a default data to save to the file 'theme.json'
	 * 
	 * Trigger filter 'ann_filter_gutenberg_fse_theme_json_data' to allow other modules change data.
	 * 
	 * @return array  Data for the 'theme.json'
	 */
	function ann_gutenberg_fse_theme_json_data() {
		...
		$data = array(
					...
					"customTemplates" => array(
						array(
							"name"      => "blog",
							"title"     => esc_html__( "Blog archive", 'ann' ),
							"postTypes" => array(
								"page"
							)
						)
					...
					"templateParts" => array(
						array(
							"name"  => "header",
							"title" => esc_html__( "Header for FSE", 'ann' ),
							"area"  => ANN_FSE_TEMPLATE_PART_AREA_HEADER,
						),
						array(
							"name"  => "sidebar",
							"title" => esc_html__( "Sidebar for FSE", 'ann' ),
							"area"  => ANN_FSE_TEMPLATE_PART_AREA_SIDEBAR,
						),
						array(
							"name"  => "content-404",
							"title" => esc_html__( "Page 404 content", 'ann' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-archive",
							"title" => esc_html__( "No posts found", 'ann' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-search",
							"title" => esc_html__( "Nothing matched search", 'ann' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-standard",
							"title" => esc_html__( "Blog post item: Standard", 'ann' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-header",
							"title" => esc_html__( "Blog post header", 'ann' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-pagination",
							"title" => esc_html__( "Blog pagination", 'ann' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "footer",
							"title" => esc_html__( "Footer for FSE", 'ann' ),
...
*/
		...
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-text',
				"name"   => esc_html__( 'Background and text color', 'ann' )
			);
		}
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text_dark'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-dark',
				"name"   => esc_html__( 'Background and dark color', 'ann' )
			);
		}
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text_link'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-link',
				"name"   => esc_html__( 'Background and link color', 'ann' )
			);
		}
		...
		if ( ! empty( $scheme['text_hover'] ) && ! empty( $scheme['text_link'] ) ) {
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Vertical from link color to hover color', 'ann' ),
			);
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Diagonal from link color to hover color', 'ann' ),
			);
		}
/*
		// Add fonts
...
*/
		...
		return apply_filters( 'ann_filter_gutenberg_fse_theme_json_data', $data );
	}
}
