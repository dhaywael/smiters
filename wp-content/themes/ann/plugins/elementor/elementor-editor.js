...
jQuery( document ).ready(
	function() {
		...
		ann_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="ann_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.ann_post_editor' );
						if ( link.length > 0 ) {
							ann_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.ann_post_editor', function(e) {
						ann_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('ann_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function ann_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
